    ...
    <div class="relative">
        ...
        <div class="mt-1 relative rounded-md shadow-sm w-full">
            <div v-if="icon" class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <slot name="icon">
                    <svg class="h-3 w-3" :class="[this.value.length ? 'text-white': 'text-gray-500']" viewBox="0 0 24 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            <input ref="input"
                   ...
                   :class="{'bg-teal-500 text-white': this.value.length && ! this.isOpen}"
                   v-model="inputValue"
                   placeholder="All"
                   data-target
                   @keydown.esc="close"

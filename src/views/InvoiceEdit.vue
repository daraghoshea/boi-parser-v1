<template>
    <app-layout>
        ...
        <main class="-mt-8 -mx-6 lg:-mt-12 lg:-mx-12 pt-12 pb-24 px-8 mx-auto pdf-wrapper h-full">
            ...
            <invoice-form v-else :id="invoice.id" :populate="populate" @saved="onSave" @error="onError"></invoice-form>
        </main>
        <toast v-if="toast.show" @hidden="hideToast">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if( this.invoice ){
                this.populate = clone(this.invoice.data)
            }
        },
        watch: {
            '$route': 'fetch'
        ...
        methods: {
            ...
            fetch() {
                this.error = this.invoice = null;
                this.invoice = this.find(
                    Number.parseInt(this.$route.params.id)
                );

                if( ! this.invoice ) {
                    this.error = `Cannot find invoice with id ${this.$route.params.id}`
                }
            },
            onSave() {
